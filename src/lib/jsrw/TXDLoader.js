  ...
  parse(input) {
    ...
    for (let i = 0; i < textureCount; i += 1) {
      try {
        ...
        if (!texture) continue;
        this.textures.set(texture.name.toLowerCase(), {
          texture: texture.texture,
          hasAlpha: texture.hasAlpha,
          compression: texture.compression,
          compressionName: texture.compressionName,
          d3dFormat: texture.d3dFormat,
          rasterFormat: texture.rasterFormat,
          platformId: texture.platformId,
          width: texture.width,
          height: texture.height,
        });
      } catch (error) {
        console.warn(`TXDLoader: Failed to load texture ${i}:`, error.message);
      }
    }
